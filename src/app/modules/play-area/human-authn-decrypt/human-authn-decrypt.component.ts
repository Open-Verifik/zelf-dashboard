...
@Component({
	selector: "app-human-authn-decrypt",
	templateUrl: "./human-authn-decrypt.component.html",
...
export class HumanAuthnDecryptComponent implements OnInit {
	readonly decryptMethod = "POST";
	readonly decryptPath = "/api/human-authn/decrypt";
	readonly decryptUrl = `${environment.apiUrl}/api/human-authn/decrypt`;
	readonly expectedShape = `{
  "publicData": { "name": "John Doe" },
  "metadata": { "source": "play-area" }
}`;
	readonly emptyHint = "Unlocks metadata. Face must match Create. Password only if you set one.";
	readonly nextSteps: HumanAuthnNextStep[] = [HUMAN_AUTHN_NEXT_CREATE];
	form: FormGroup;
	loading = false;
	...
	buildPayload(): Record<string, unknown> {
		...
		const payload: Record<string, unknown> = {
			faceBase64: this.humanPay.normalizeFaceBase64ForApi(fv.faceBase64 || ""),
			zelfProof: (fv.zelfProof || "").toString().trim(),
			os: fv.os,
		};
		if (fv.password?.trim()) payload.password = fv.password.trim();
		if (fv.livenessLevel?.trim()) payload.livenessLevel = fv.livenessLevel.trim();
		if (fv.verifierKey?.trim()) payload.verifierKey = fv.verifierKey.trim();
		return payload;
	}
	...
	onSelfieFileSelected(event: Event): void {
		...
		reader.onload = () => {
			...
			this.form.patchValue({ faceBase64: dataUrl });
			this.selfiePreviewUrl = dataUrl;
			this.selfieFileName = file.name;
			this.form.get("faceBase64")?.markAsDirty();
		};
		reader.onerror = () => this.snackBar.open("Failed to read image", "Close", { duration: 3000 });
		reader.readAsDataURL(file);
	}

	clearSelfie(): void {
		this.form.patchValue({ faceBase64: "" });
		this.selfiePreviewUrl = null;
		this.selfieFileName = null;
	}

	async runDecrypt(isPolling = false): Promise<boolean> {
		const fv = this.form.value;
		if (!fv.faceBase64?.trim() || !fv.zelfProof?.trim()) {
			if (!isPolling) this.snackBar.open("Selfie and zelfID are required", "Close", { duration: 3500 });
			return false;
		}
		...
		try {
			...
			this.snackBar.open("Decrypt successful!", "Close", { duration: 5000 });
	...
	async pollForVerification(attempts = 0): Promise<void> {
		...
		const ok = await this.runDecrypt(true);
		if (ok) {
			this.humanPay.paymentProcessing = false;
	...
	async onBiometricSuccess(data: BiometricData): Promise<void> {
		...
		const amount = pd?.paymentDetails?.cost ?? 0.05;
	...
	async submitPayment(biometricData: BiometricData, amount: number): Promise<void> {
		...
		this.showBiometricModal = false;
		this.snackBar.open("Payment successful! Retrying decrypt...", "Close", { duration: 3000 });
